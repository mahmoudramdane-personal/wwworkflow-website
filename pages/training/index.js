...
export default function TrainingHome() {
  ...
  async function fetchProgress() {
    if (!intern) return;
    try {
      const r = await fetch(`/api/training/track?internId=${intern.name}`);
      const data = await r.json();
      // Also fetch all progress for dashboard
      const r2 = await fetch('/api/training/track?listAll=true');
      setAllProgress(await r2.json());
    } catch (e) {}
  }

  useEffect(() => {
    if (activeTab === 'admin' && intern) fetchProgress();
  }, [activeTab, intern]);

  function handleLogout() {
    localStorage.removeItem('training_intern');
  ...
  return (
    <>
      ...
      <div className="bg-black text-white px-6 py-14 md:py-16 text-center">
        <h1 className="font-eb text-4xl md:text-5xl text-white mb-2">🎓 Training</h1>
        <p className="font-alt text-cgray">
          Welcome, <span className="text-white font-medium">{intern?.displayName}</span>
        </p>
        <button onClick={handleLogout} className="font-alt text-xs text-cgray hover:text-white mt-2 underline transition">
          Not you? Sign out
        </button>
      </div>
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6">
        ...
        <div className="bg-white border border-t-0 border-gray-300 rounded-b-lg px-6 py-8">
          {activeTab === 'player' && (
            <div>
              <p className="font-alt text-gray-500 mb-6 text-sm">Select a video to start:</p>
              <div className="space-y-3">
                {videos.map((v, i) => (
                  <Link key={v.id} href={`/training/video/${v.id}`}
                    className="flex items-center gap-4 bg-cgrey hover:bg-gray-200 transition rounded-lg p-4 group">
                    <span className="flex-shrink-0 w-8 h-8 rounded-full bg-blue text-white flex items-center justify-center text-sm font-ibm">{i + 1}</span>
                      ...
                      <div key={internId} className="bg-cgrey rounded-lg p-5 border border-gray-200">
                        <div className="flex items-center justify-between mb-4">
                          ...
                          <div className="text-right">
                            <div className={`font-eb text-2xl ${avg >= 80 ? 'text-green-600' : avg > 0 ? 'text-blue' : 'text-gray-400'}`}>
                              {avg}%
                            </div>
                            <p className="font-alt text-xs text-gray-500">avg</p>
                          </div>
                        </div>
                        {entries.map(([videoId, data]) => (
                          <div key={videoId} className="mb-3 last:mb-0">
                            ...
                            <div className="w-full bg-gray-200 rounded-full h-2.5 overflow-hidden">
                              <div className={`h-full rounded-full transition-all ${data.completed ? 'bg-green-500' : data.progress > 0 ? 'bg-blue' : 'bg-gray-300'}`}
